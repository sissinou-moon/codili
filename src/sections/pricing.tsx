...
export default function Pricing() {
    ...
    return (
        <section className='bg-white lg:px-18 md:px-10 px-7 py-7 pb-20'>
            <motion.div variants={container} whileInView='show' initial='hidden' viewport={{once: true, amount: 0.2}} className='flex flex-col bg-white justify-center items-center text-center'>
                ...
                <div className='flex md:flex-row flex-col gap-5 md:gap-2 lg:gap-5 justify-center items-center mt-10'>
                    {pricing.map((item, i) => (
                        <motion.div variants={cardAnimation} className='flex flex-col max-w-80 border border-black/20 hover:border-[#87A2FF] rounded-2xl px-7 lg:px-7 md:px-3 py-5 justify-start items-start transition-all duration-300 ease-in-out hover:scale-101' key={i}>
                            <p className='text-black text-md'>{item.name}</p>
                            <p className='text-2xl text-black font-semibold mt-3'>${item.price}</p>
                            <p className='text-black/70 md:text-black font-light text-sm md:text-[12px] lg:text-[14px] text-start mt-3'>{item.description}</p>
                            ...
                            <div className='flex flex-col gap-4 mt-10 mb-2'>
                                {item.services.map((service, index) => (
                                    <div className='flex flex-row gap-2 items-center' key={index}>
                                        ...
                                        <p className='text-sm md:text-[10px] lg:text-sm text-start text-black '>{service}</p>
                                    </div>
                                ))}
                            </div>
                        </motion.div>
